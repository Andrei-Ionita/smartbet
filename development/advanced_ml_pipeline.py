...
class AdvancedMLPipeline:
    ...
    def feature_engineering(self, df):
        ...
        print("2️⃣ Creating team form features...")
        ...
        for team in df['home_team'].unique():
            ...
            for _, match in team_away_matches.iterrows():
                all_team_matches.append({
                    ...
                    'result': 1 if match['away_score'] > match['home_score'] else (0 if match['away_score'] == match['home_score'] else -1)
                })
            
            # Sort by date
            all_team_matches = sorted(all_team_matches, key=lambda x: x['date'])
        
        # 3. GOAL DIFFERENCE AND ATTACKING/DEFENSIVE STRENGTH
        print("3️⃣ Creating goal difference and strength features...")
        ...
        # 4. TEMPORAL FEATURES
        print("4️⃣ Creating temporal features...")
        ...
        # 5. BOOKMAKER VARIANCE FEATURES
        print("5️⃣ Creating bookmaker variance features...")
    ...
    def create_time_splits(self, df):
        ...
        val_mask = df['season_year'] == 2024
        
        # Test: 2024-2025 season (most recent)
        test_mask = df['season_name'] == '2024/2025'
        
        print(f"📊 Training set: {train_mask.sum():,} matches (seasons 2021-2023)")
        print(f"📊 Validation set: {val_mask.sum():,} matches (season 2024)")
        print(f"📊 Test set: {test_mask.sum():,} matches (season 2024-2025)")
    ...
    def save_models(self):
        ...
        # Save LightGBM
        lgb_filename = f'lightgbm_model_{timestamp}.txt'
        self.model_lgb.save_model(lgb_filename)
        
        # Save XGBoost
        xgb_filename = f'xgboost_model_{timestamp}.json'
