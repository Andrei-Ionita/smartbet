...
class SerieAHoldoutValidator:
    ...
    def create_holdout_split(self):
        ...
        # Sort by season_name to ensure proper chronological split
        self.df = self.df.sort_values('season_name').reset_index(drop=True)
        ...
        holdout_seasons = self.holdout_df['season_name'].value_counts().sort_index()
    ...
    def generate_predictions(self):
        ...
        try:
            ...
            # LightGBM class mapping for Serie A dataset: check target values
            target_values = sorted(self.holdout_df['target'].unique())
            print(f"📋 Target values in dataset: {target_values}")
            
            # Assuming 0=home, 1=away, 2=draw based on typical encoding
            class_mapping = {0: 'home', 1: 'away', 2: 'draw'}
            text_classes = ['home', 'away', 'draw']
            print(f"📋 Class mapping: {class_mapping}")
            
        except Exception as e:
            raise Exception(f"❌ Prediction failed: {e}")
        
        # Create predictions dataframe with correct column names
        pred_df = self.holdout_df[['home_team', 'away_team', 'season_name', 
                                  'target', 'avg_home_odds', 'avg_draw_odds', 'avg_away_odds']].copy()
        ...
        for i, pred_outcome in enumerate(pred_df['predicted_outcome']):
            if pred_outcome == 'home':
                predicted_odds.append(pred_df.iloc[i]['avg_home_odds'])
            elif pred_outcome == 'draw':
                predicted_odds.append(pred_df.iloc[i]['avg_draw_odds'])
            else:  # away
                predicted_odds.append(pred_df.iloc[i]['avg_away_odds'])
        
        pred_df['predicted_odds'] = predicted_odds
        
        # Convert target to text for comparison (assuming 0=home, 1=away, 2=draw)
        target_mapping = {0: 'home', 1: 'away', 2: 'draw'}
        pred_df['actual_result'] = pred_df['target'].map(target_mapping)
        
        # Determine correct predictions
        pred_df['correct'] = (pred_df['predicted_outcome'] == pred_df['actual_result']).astype(int)
