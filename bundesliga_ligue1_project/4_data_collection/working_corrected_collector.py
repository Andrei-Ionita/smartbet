#!/usr/bin/env python3
"""
WORKING CORRECTED Bundesliga and Ligue 1 Data Collector
Fixed state filtering (FT instead of FINISHED) and corrected score parsing
"""
...
class WorkingCorrectedCollector:
    def __init__(self):
    ...
    def collect_season_fixtures(self, league_id, season_id, season_name):
        ...
        while True:
            ...
            # Filter for completed fixtures only (CORRECTED: FT not FINISHED!)
            completed_fixtures = [
                f for f in fixtures 
    ...
    def collect_all_data(self):
        ...
        self.log("🚀 STARTING WORKING CORRECTED DATA COLLECTION")
        self.log("📊 Using FT state filtering + corrected score parsing")
        self.log("=" * 70)
        ...
        filename = f"working_corrected_bundesliga_ligue1_data_{timestamp}.csv"
        df.to_csv(filename, index=False)
        
        # Summary
        self.log(f"\n📊 WORKING CORRECTED COLLECTION SUMMARY:")
...
def main():
    ...
    try:
        collector = WorkingCorrectedCollector()
        df, filename = collector.collect_all_data()
        
        if df is not None:
            print(f"\n🎉 SUCCESS: Working corrected data collection completed!")
