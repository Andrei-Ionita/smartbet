#!/usr/bin/env python3
"""
CORRECTED Bundesliga and Ligue 1 Data Collector
Fixes the SportMonks API score parsing to capture all three outcomes properly
"""
...
class CorrectedBundesligaLigue1Collector:
    def __init__(self):
        ...
        # League configurations with correct IDs (verified)
        self.leagues = {
            'bundesliga': {
                'league_id': 82,
                'name': 'Bundesliga',
    ...
    def get_league_seasons(self, league_id, league_name, target_seasons):
        ...
        self.log(f"🔍 Finding target seasons for {league_name}...")
        
        # Get all seasons
        seasons_data = self.make_api_request("seasons", {
        ...
        for season in seasons:
            ...
            if season_name in target_seasons:
                target_season_data.append(season)
                self.log(f"   ✅ Found {season_name} (ID: {season['id']})")
        
        return target_season_data
    ...
    def extract_fixture_features(self, fixture, league_name):
        ...
        try:
            ...
            # Determine outcome (1X2) - NOW CORRECTLY!
    ...
    def collect_all_data(self):
        ...
        self.log("🚀 STARTING CORRECTED BUNDESLIGA AND LIGUE 1 DATA COLLECTION")
        self.log("=" * 70)
        ...
        filename = f"corrected_bundesliga_ligue1_training_data_{timestamp}.csv"
        ...
        if not df.empty:
            self.log(f"\n📋 SAMPLE DATA:")
            sample_data = df[['league', 'home_team', 'away_team', 'home_goals', 'away_goals', 'outcome']].head()
...
def main():
    ...
    try:
        collector = CorrectedBundesligaLigue1Collector()
        df, filename = collector.collect_all_data()
        
        if df is not None:
            print(f"\n✅ SUCCESS: Corrected data collection completed!")
            ...
            if len(outcome_counts) == 3:
                print("🎉 ALL THREE OUTCOMES (0,1,2) SUCCESSFULLY CAPTURED!")
            else:
                print("⚠️  Still missing some outcome types")
        else:
