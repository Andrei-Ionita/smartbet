...
const COPY = {
  en: {
    back: 'All strategies', hypothesis: 'Strategy hypothesis', mechanics: 'How it works', useful: 'When the idea may be useful', risks: 'Where it can fail', settlement: 'Settlement', example: 'Worked example', breakEven: 'Price check', evidence: 'Current lab evidence', evidenceBody: 'This counter contains forward-settled decisions from this exact strategy version. A small sample is not proof, and retrospective results cannot qualify a strategy.', boundaryTitle: 'What this page is—and is not', boundaryBody: 'This is an educational explanation of a market hypothesis. It is not a current pick, a promise of profit, personalized advice or evidence that the strategy works.', inspect: 'Inspect live fixtures', methodology: 'Read the testing methodology', related: 'Continue learning',
  },
  ro: {
    back: 'Toate strategiile', hypothesis: 'Ipoteza strategiei', mechanics: 'Cum funcționează', useful: 'Când poate fi utilă ideea', risks: 'Unde poate eșua', settlement: 'Decontare', example: 'Exemplu explicat', breakEven: 'Verificarea prețului', evidence: 'Dovezi curente din laborator', evidenceBody: 'Contorul include decizii forward încheiate din această versiune exactă. Un eșantion mic nu este dovadă, iar rezultatele retrospective nu pot califica strategia.', boundaryTitle: 'Ce este și ce nu este această pagină', boundaryBody: 'Aceasta este o explicație educativă a unei ipoteze de piață. Nu este o selecție curentă, o promisiune de profit, un sfat personalizat sau dovada că strategia funcționează.', inspect: 'Analizează meciurile curente', methodology: 'Citește metodologia de testare', related: 'Continuă să înveți',
  },
}
...
export default function StrategyDetailContent({ strategy }: { strategy: StrategyDefinition }) {
  ...
  return (
    <article className="mx-auto max-w-5xl pb-12">
      ...
      <section className="mt-6 rounded-2xl border border-violet-200 bg-violet-50 p-6 sm:p-8"><div className="flex items-start gap-4"><FlaskConical className="mt-1 h-6 w-6 shrink-0 text-violet-700" /><div><h2 className="text-lg font-black text-violet-950">{c.evidence}</h2><p className="mt-2 text-sm leading-6 text-violet-900">{c.evidenceBody}</p><Link href="/methodology" className="mt-4 inline-flex min-h-[44px] items-center gap-2 font-bold text-violet-800 hover:text-violet-950">{c.methodology}<ArrowRight className="h-4 w-4" /></Link></div></div></section>
