...
const COPY = {
  en: {
    ...
    advanced: 'Advanced evidence', methodology: 'Methodology', calibration: 'Probability calibration', unavailable: 'Results are temporarily unavailable.',
  },
  ro: {
    ...
    advanced: 'Dovezi avansate', methodology: 'Metodologie', calibration: 'Calibrarea probabilităților', unavailable: 'Rezultatele sunt temporar indisponibile.',
  },
} as const
...
export default function UnifiedResultsContent() {
  ...
  return <div className="min-h-screen bg-slate-50"><header className="border-b border-slate-200 bg-white"><div className="mx-auto max-w-7xl px-4 py-10 sm:px-6"><p className="text-xs font-black uppercase tracking-[0.2em] text-blue-700">{c.eyebrow}</p><h1 className="mt-3 text-4xl font-black tracking-tight text-slate-950 sm:text-5xl">{c.title}</h1><p className="mt-4 max-w-3xl text-lg leading-8 text-slate-600">{c.intro}</p></div></header><main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12"><nav aria-label={c.eyebrow} className="flex gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-2">{tabs.map(tab => { const Icon = tab.icon; const on = category === tab.key; return <button key={tab.key} type="button" onClick={() => setCategory(tab.key)} className={`inline-flex min-h-11 shrink-0 items-center gap-2 rounded-xl px-4 text-sm font-black ${on ? 'bg-slate-950 text-white' : 'text-slate-600 hover:bg-slate-100'}`}><Icon className="h-4 w-4" />{c.tabs[tab.key]}</button> })}</nav>
    ...
    <section className="mt-8 rounded-2xl border border-blue-100 bg-blue-50 p-5 sm:p-6"><div className="flex items-start gap-3"><Info className="mt-0.5 h-5 w-5 text-blue-700" /><div><h2 className="font-black text-blue-950">{c.advanced}</h2><p className="mt-2 text-sm leading-6 text-blue-900">{c.policy}</p><div className="mt-3 flex flex-wrap gap-4 text-sm font-bold"><Link href="/methodology" className="text-blue-800 hover:underline">{c.methodology} →</Link><Link href="/calibration" className="text-blue-800 hover:underline">{c.calibration} →</Link><Link href="/proof/anchors" className="text-blue-800 hover:underline">Technical verification →</Link></div></div></div></section></main></div>
}
